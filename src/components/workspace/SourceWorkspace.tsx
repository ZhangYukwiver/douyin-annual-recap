  ...
  return (
    <View testID="setup-workspace" style={styles.root}>
      <View style={[styles.topbar, phone && styles.topbarPhone]}>
        ...
        <View style={styles.topStatus}><View style={[styles.statusDot, connected && styles.statusDotReady]} /><Text numberOfLines={1} style={styles.statusText}>{busy && ready ? `${source} · 采集中，报告用采集前的数据` : source}</Text></View>
        <Pressable accessibilityRole="button" disabled={!ready} onPress={onEnterWorkspace} style={({ pressed }) => [styles.enter, !ready && styles.disabled, pressed && styles.pressed, pointer]}><Text style={styles.enterText}>{phone ? "进入" : "打开报告"}</Text><ArrowRight color={color.buttonText} size={17} /></Pressable>
      </View>
      ...
      <ScrollView contentContainerStyle={[styles.scrollContent, mobile && styles.scrollContentMobile]} showsVerticalScrollIndicator={false}>
        <View style={[styles.layout, mobile && styles.layoutMobile]}>
          <View style={[styles.intro, mobile && styles.introMobile]}>
            ...
            <Text style={[styles.title, phone && styles.titlePhone]}>{copy.title[0]}{phone ? "\n" : ""}{copy.title[1]}</Text>
            <Text style={styles.lead}>{copy.lead}</Text>
            <View style={styles.seal}><ShieldCheck color={color.accent} size={31} strokeWidth={1.2} /><Text style={styles.sealText}>{copy.seal}</Text><Text style={styles.sealYear}>{new Date().getFullYear()}</Text></View>
          ...
          <View style={styles.operations}>
            ...
            <View style={[styles.connection, mobile && styles.connectionMobile]}>
              ...
              <View style={styles.connectionAction}><Text style={styles.actionKicker}>{connected ? "CONNECTION READY" : "AUTO PAIR LOCAL SERVICE"}</Text><Text style={styles.actionValue}>{connected ? "连接正常" : "等待连接"}</Text><Text style={styles.actionMeta}>{connected ? collectorUrl : "连接后会读取现有本地快照，不会自动上传。"}</Text><Pressable accessibilityRole="button" disabled={busy} onPress={() => void (connected ? onDisconnect() : onConnect())} style={({ pressed }) => [styles.primary, busy && styles.disabled, pressed && styles.pressed, pointer]}>{busy ? <ActivityIndicator color={color.buttonText} size="small" /> : connected ? <Unplug color={color.buttonText} size={17} /> : <Link2 color={color.buttonText} size={17} />}<Text style={styles.primaryText}>{connected ? "断开连接" : "连接采集器"}</Text></Pressable></View>
            </View>
...
function Step({ done, detail, index, label }: { done: boolean; detail: string; index: string; label: string }) { return <View style={styles.step}><View style={[styles.stepIndex, done && styles.stepIndexDone]}>{done ? <Check color={color.buttonText} size={14} strokeWidth={3} /> : <Text style={styles.stepIndexText}>{index}</Text>}</View><View style={styles.flex}><Text style={styles.stepLabel}>{label}</Text><Text style={styles.stepDetail}>{detail}</Text></View></View>; }
function Count({ icon: CountIcon, label, value }: { icon: React.ComponentType<{ color?: string; size?: number }>; label: string; value: number }) { return <View style={styles.count}><CountIcon color={color.accent} size={18} /><View><Text style={styles.countValue}>{value.toLocaleString("zh-CN")}</Text><Text style={styles.countLabel}>{label}</Text></View></View>; }
function ActionButton({ busy, disabled, icon: ActionIcon, label, onPress }: { busy?: boolean; disabled: boolean; icon: React.ComponentType<{ color?: string; size?: number }>; label: string; onPress: () => void }) { return <Pressable accessibilityRole="button" disabled={disabled} onPress={onPress} style={({ pressed }) => [styles.action, disabled && styles.disabled, pressed && styles.pressed, pointer]}>{busy ? <ActivityIndicator color={color.accent} size="small" /> : <ActionIcon color={color.textSecondary} size={17} />}<Text style={styles.actionText}>{label}</Text></Pressable>; }
