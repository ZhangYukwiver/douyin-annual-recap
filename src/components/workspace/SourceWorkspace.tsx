...
import { REPORT_STYLES, type ReportStyle } from "../../services/reportStyle";
...
export interface SetupWorkspaceProps {
  ...
  onChangeReportStyle: (style: ReportStyle) => void;
}
      ...
      <ScrollView contentContainerStyle={[styles.scrollContent, mobile && styles.scrollContentMobile]} showsVerticalScrollIndicator={false}>
        <View style={[styles.layout, mobile && styles.layoutMobile]}>
          <View style={[styles.intro, mobile && styles.introMobile]}>
            ...
            {Platform.OS === "web" ? <View testID="report-style" style={styles.styleBlock}><Text style={styles.stepLabel}>03 · 报告风格</Text><Text style={styles.stepDetail}>点“打开报告”时使用的版式</Text><View style={styles.styleRow}>{REPORT_STYLES.map((item) => <Pressable key={item.key} accessibilityRole="radio" aria-checked={reportStyle === item.key} onPress={() => onChangeReportStyle(item.key)} style={({ pressed }) => [styles.styleOption, reportStyle === item.key && styles.styleOptionOn, pressed && styles.pressed, pointer]}><Text style={[styles.styleLabel, reportStyle === item.key && styles.styleLabelOn]}>{item.label}</Text><Text style={styles.styleMeta}>{item.detail}</Text></Pressable>)}</View></View> : null}
          </View>
...
const styles = StyleSheet.create({
  ...
  styleBlock: { marginTop: 18, paddingTop: 18, borderTopWidth: 1, borderTopColor: "#302D28" }, styleRow: { flexDirection: "row", gap: 8, marginTop: 10 }, styleOption: { flex: 1, minHeight: 52, justifyContent: "center", paddingHorizontal: 10, paddingVertical: 8, borderWidth: 1, borderColor: "#3E3A32" }, styleOptionOn: { borderColor: "#C5A161", backgroundColor: "#1A1710" }, styleLabel: { color: "#A9A39A", fontSize: 11, fontWeight: "800" }, styleLabelOn: { color: "#F1E9DA" }, styleMeta: { color: "#777067", fontSize: 9, marginTop: 3 },
